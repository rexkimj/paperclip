...
export async function testEnvironment(
  ...
  if (isNonEmpty(configOpenAiKey) || isNonEmpty(hostOpenAiKey)) {
    ...
    checks.push({
      ...
      level: "info",
      message: "OPENAI_API_KEY is set for Ollama/Codex authentication.",
    ...
    if (codexAuth) {
      checks.push({
        ...
        level: "info",
        message: "Ollama (via codex) is authenticated via its own auth configuration.",
      ...
      checks.push({
        ...
        level: "warn",
        message: "OPENAI_API_KEY is not set. Runs may fail until authentication is configured if required.",
        hint: "Set OPENAI_API_KEY in adapter env, shell environment, or run `codex auth` to log in if using codex.",
      });
    }
  }
    ...
    if (!commandLooksLike(command, "codex")) {
      ...
      if (probe.timedOut) {
        checks.push({
          ...
          level: "warn",
          message: "Ollama hello probe timed out.",
          hint: "Retry the probe. If this persists, verify you can run `Respond with hello` from this directory manually.",
        });
        ...
        checks.push({
          ...
          level: "warn",
          message: "Ollama/codex CLI is installed, but authentication is not ready.",
          ...(detail ? { detail } : {}),
